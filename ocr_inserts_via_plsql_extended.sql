...
ALTER TABLE Accounts
    DROP CONSTRAINT Accounts_ext_ratings;
...
DROP SEQUENCE seq_ext_rating_rk;
...
CREATE TABLE Accounts ( 
    ...
    ext_rating_rk number(20,0)  NOT NULL,
...
CREATE TABLE Ext_ratings ( 
    ext_rating_rk number(20,0)  NOT NULL, 
    ext_rating_id varchar2(20)  NOT NULL, 
    rating_grade char(3)  NOT NULL, 
    pd number(20,4)  NOT NULL, 
    description varchar2(150)  NOT NULL, 
    CONSTRAINT ext_ratings_pk PRIMARY KEY (ext_rating_rk) 
) 
;
...
ALTER TABLE Accounts ADD CONSTRAINT Accounts_ext_ratings 
    FOREIGN KEY (ext_rating_rk) 
    REFERENCES ext_ratings (ext_rating_rk)
;
...
CREATE SEQUENCE seq_ext_rating_rk 
      INCREMENT BY 1 
      NOMINVALUE 
      NOMAXVALUE 
      START WITH 1 
...
insert into ext_ratings (ext_rating_rk, ext_rating_id,rating_grade,pd,description) 
values 
...
values 
(seq_int_rating_rk.nextval,'12a232s3','0.5',0.5582,'internal - IRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'212a2s3','1.0',0.03712,'internal - AIRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'453a2s3','1.5',0.1252,'internal - IRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'968a2s3','2.0',0.0182,'internal - AIRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'3212a2s3','2.5',0.12382,'internal - AIRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'9622a2s3','1.5',0.306282,'internal - IRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'21s22a2s3','1.0',0.02582,'internal - IRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'212a2s3','1.5',0.6382,'internal - AIRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'2322a2s3','2.0',0.132282,'internal - IRB')
;

insert into int_ratings (int_rating_rk, int_rating_id,rating_grade,pd,description) 
values 
(seq_int_rating_rk.nextval,'3622a2s3','2.5',0.1282,'internal - IRB')
;
...
begin 
    ...
    LOOP 
        ...
        case 
            when dbms_random.value between .01 and .19 
                then 'ST1' 
            when dbms_random.value between .4 and .69 
                then 'ST2'
            else 
                     'ST3' 
        end, --type
...
begin 
    ...
    LOOP 
        insert into collaterals (COLLATERAL_RK, COLLATERAL_ID, COLLATERAL_TYPE, EXPIRATION_DATE, EFFECTIVE_DATE, COLLATERAL_AMT, COLLATERAL_CURRENCY) 
        values 
        (seq_collateral_rk.nextval, 
        substr(standard_hash(to_char(round(dbms_random.value*100000,0)), 'MD5'),1,5) || '*' || to_char(round(dbms_random.value*100000,0)) || '*COL', 
        case 
		...
		sysdate + round(dbms_random.value*1000,0), --expiration_date
		sysdate - round(dbms_random.value*100,0), --effective_date
        round(dbms_random.value*100000,2), --amt
        case 
            when dbms_random.value between .01 and .14 then 'HRK' 
            when dbms_random.value between .15 and .28 then 'EUR'
            when dbms_random.value between .29 and .43 then 'BAM'
...
declare
    v_ext_rating_rk number;
    v_int_rating_rk number;
    v_customer_rk number;
    v_product_rk number;
    v_provision_rk number;
    v_collateral_rk number;
    v_counterpart_ind varchar2(3);
begin 
    FOR i IN 1..150000 
    LOOP 
        SELECT ext_rating_rk into v_ext_rating_rk FROM ext_ratings ORDER BY DBMS_RANDOM.VALUE FETCH NEXT 1 ROWS ONLY; 
        ...
        SELECT indicator into v_counterpart_ind FROM hlp_counterparty_indicator ORDER BY DBMS_RANDOM.VALUE*weight FETCH NEXT 1 ROWS ONLY; 
        
        insert into accounts (account_rk,account_id,in_default,maturity_date,inception_date,currency,balance_amt,interest_amt,ext_rating_rk,int_rating_rk,customer_rk,product_rk,provision_rk,collateral_rk) 
        values 
        (
            seq_account_rk.nextval, 
            substr(standard_hash(to_char(round(dbms_random.value*100000,0)), 'MD5'),1,5) || '*' || to_char(round(dbms_random.value*10000,0)) || '#' || v_counterpart_ind, 
            case 
...
select 
    substr(account_id, instr(account_id, '#')+1, 10), count(*)
from 
    accounts
group by 
    substr(account_id, instr(account_id, '#'), 10)
;
